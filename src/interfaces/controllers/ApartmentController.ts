...
export class ApartmentController {
  private apartmentRepository: PostgresApartmentRepository
  private publisherRepository: PostgresPublisherRepository

  constructor() {
    this.apartmentRepository = new PostgresApartmentRepository()
    this.publisherRepository = new PostgresPublisherRepository()
  }
  ...
  async getAllApartments(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (req.query.status && Object.values(ApartmentStatus).includes(req.query.status as ApartmentStatus)) {
        filters.status = req.query.status as ApartmentStatus
      }

      if (req.query.type && Object.values(ApartmentType).includes(req.query.type as ApartmentType)) {
        filters.type = req.query.type as ApartmentType
      }
  ...
  async createApartment(req: Request, res: Response): Promise<void> {
    try {
      const apartmentData = req.body.apartment as Apartment
      const publisherData = req.body.publisher as {
        name: string
        email: string
        phoneNumber?: string
        password: string
