...
class PostgresApartmentRepository {
      ...
      `;
        ...
        for (const apartment of apartments) {
            ...
            if (apartment.benefits && apartment.benefits.length > 0) {
                const { rows: benefitRows } = await index_1.pool.query(`SELECT name FROM benefit WHERE id IN (${apartment.benefits.join(",")})`);
                apartment.benefitNames = benefitRows.map((row) => row.name);
            }
        }
        for (const apartment of apartments) {
            if (apartment.neighborhoodId) {
                const { rows: neighborhoodRows } = await index_1.pool.query("SELECT name FROM neighborhood WHERE id = $1", [
    ...
    async findById(id) {
        ...
        // Get city name
        if (apartment.cityId) {
            const { rows: cityRows } = await index_1.pool.query("SELECT name FROM city WHERE id = $1", [apartment.cityId]);
        ...
        if (apartment.publisherId) {
            const { rows: publisherRows } = await index_1.pool.query("SELECT id, name, email, phone_number, created_at FROM publisher WHERE id = $1", [apartment.publisherId]);
    ...
    async create(apartment) {
        ...
        RETURNING *`, [
            ...
            if (newApartment.cityId) {
                const { rows: cityRows } = await index_1.pool.query("SELECT name FROM city WHERE id = $1", [newApartment.cityId]);
                if (cityRows.length > 0) {
